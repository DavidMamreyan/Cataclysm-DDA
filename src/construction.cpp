        ...
        int filter_mode )
{
    ...
    for( construction &it : constructions ) {
        ...
        bool ok = false;
        // filter_mode: 0 = all,
        // 1 = ready (skills and materials satisfied, but location not satisfied),
        // 2 = buildable here (skills, materials, and location satisfied)
        if( filter_mode == 0 ) {
            ok = true;
...
construction_id construction_menu( const bool blueprint )
{
    ...
    // filter_mode: 0 = all, 1 = ready (skills and materials satisfied, but location not satisfied),
    // 2 = buildable here (skills, materials, and location satisfied)
    ...
    const auto recalc_buffer = [&]() {
        ...
        if( !constructs.empty() ) {
            ...
            for( std::vector<construction *>::iterator it = options.begin();
                ...
                if( filter_mode == 1 ) {
                    // Show stages where skills and materials are satisfied but location is not.
                    if( !( player_can_build( player_character, total_inv, *current_con, true ) &&
                           !can_construct( *current_con ) ) ) {
                        continue;
                    }
                } else if( filter_mode == 2 ) {
                    // Show stages where skills, materials, and location are satisfied.
                    if( !( player_can_build( player_character, total_inv, *current_con, true ) &&
                           can_construct( *current_con ) ) ) {
                        continue;
                    }
